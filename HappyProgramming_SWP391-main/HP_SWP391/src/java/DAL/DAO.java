...
public class DAO {
    ...
    private ArrayList<MentorProfile1> mpList1;
    ...
    public MentorProfile getMentorByUID(int uid) {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                return new MentorProfile(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6),rs.getString(7));
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
...
//    public SkillCategory getMentorProfileByMentorID(int mentorID) {
//        String sql1 = "select * from MentorProfile where mentorID = ?";
//        String sql2 = "select * from from MentorSkill where mentorID = ?";
//        String sql3 = "select * from from SkillCategory where skillID = ?";
//        MentorProfile mp = new MentorProfile();
...
//
//            PreparedStatement ps2 = con.prepareStatement(sql2);
//            ps2.setInt(2, mentorID);
//            ResultSet rs2 = ps2.executeQuery();
//            while (rs2.next()) {
//                ms= new MentorSkill(rs2.getInt(1), rs2.getInt(2), rs2.getInt(3), rs2.getInt(4),rs2.getString(5));
//            }
//            PreparedStatement ps3 = con.prepareStatement(sql3);
//            ps3.setInt(2, ms.getSkillID());
//            ResultSet rs3 = ps3.executeQuery();
//            while (rs3.next()) {
//                return new SkillCategory(rs3.getInt(1), rs3.getString(2), rs3.getBoolean(3));
//            }
//        } catch (SQLException e) {
//            System.out.println(e.getMessage());
//        }
...
//    }
 ...
 public static void main(String[] args) {
        DAO dao = new DAO();
        //System.out.println(dao.getMentorByMentorID(1));
        //ao.UpdateMentorProfile(1, "waiting", "Master of coding", "c", "viet an cut");
       dao.getSkillBySkillID(1);
    }
//    public ArrayList<MentorProfile1> getProfileMentorByMentorID(int mentorID) {
//        String sql = "select *from\n"
//                + "(select a.mentorID, a.avatar,a.introduction,a.GitHub,a.LinkedIn,sc.skillName,a.description,sc.status from\n"
//                + "(select mp.mentorID,mp.avatar,mp.introduction,mp.GitHub,mp.LinkedIn,ms.description,ms.skillID,ms.yearsOfExp\n"
//                + "from [MentorProfile] as mp Right join MentorSkill as ms \n"
//                + "on mp.mentorID= ms.mentorID ) as a , SkillCategory sc\n"
//                + "where a.skillID=sc.skillID)as b\n"
//                + "where b.mentorID=?";
//        MentorProfile1 mp1 = new MentorProfile1();
//        mpList1 = new ArrayList<>();
//        try {
//            PreparedStatement ps = con.prepareStatement(sql);
//            ps.setInt(1, mentorID);
//            ResultSet rs = ps.executeQuery();
//            while (rs.next()) {
//               mpList1.add(new MentorProfile1(rs.getInt(1), rs.getString(2),
//                        rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6),rs.getString(7),rs.getBoolean(8)));
//                
//            }
//        } catch (SQLException e) {
//            System.out.println(e.getMessage());
...
//        return mpList1;
//    }
    public MentorProfile getMentorByMentorID(int mentorID) {
        String query = "select * from MentorProfile where mentorID= ?";
        try {
            PreparedStatement ps = con.prepareStatement(query);
            ps.setInt(1,mentorID );
            ResultSet rs = ps.executeQuery();
     ...
     public ArrayList<MentorProfile1> getProfileMentorByMentorID(int mentorID) {
        String sql = "select sc.skillID ,sc.skillName,ms.description,ms.yearsOfExp,sc.status\n"
                + "from SkillCategory sc right join MentorSkill ms\n"
                + "on sc.skillID=ms.skillID\n"
                + "where ms.mentorID = ?"
                + "\n";             
     ...
     public void UpdateMentorProfile(int mentorID, String avatar, String introduction, String LinkedIn, String GitHub,String Profession) {
        String sql = "Update MentorProfile\n"
                + "Set avatar = ?, introduction = ?,LinkedIn =? ,GitHub=?,Profession=?\n"
                + "where mentorID = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
     ...
     public void UpdateSkillCategory(String SkillName, String status, int skillID) {
        String sql = "update SkillCategory\n" +
                    "set skillName = ?, status = ?\n" +
                    "where skillID = ? ";
        try {
     ...
     public void UpdateMentorSkill(String description, int year, int skillID) {
        String sql = "Update MentorSkill "
                + "set description = ?, yearsOfExp = ?"
                + " where skillID = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
     ...
     public void UpdateMentorSkills(int id, int mentorID, String[] skill, String[] yoe, String[] desc) {
        for (String s : skill) {
